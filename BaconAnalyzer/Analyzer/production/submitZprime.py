...
if __name__ == '__main__':
    ...
    for label, isMc in samples.iteritems():
        ...
        if isMc in ['data','rereco']:
            exec_me("python baconBatch.py %s %s -a 4:%f --list 1:../lists/production12a/%s.txt --outdir /afs/cern.ch/work/c/cdillipl/PHIphiphi/Bacon/CMSSW_8_0_20/src/BaconAnalyzer/Analyzer/Phiphiphi_bits/%s/%s_%s  %s"%(executable,optionsDataMc[isMc],xsec,label,analysisDir,label,isMc,monitorOption),options.dryRun)
        else:            
            exec_me("python baconBatch.py %s %s -a 4:%f --list 1:../lists/production12/%s.txt --outdir /afs/cern.ch/user/c/cdillipl/PHIphiphi/Bacon/CMSSW_8_0_20/src/BaconAnalyzer/Analyzer/Phiphiphi_bits/%s/%s_%s  %s"%(executable,optionsDataMc[isMc],xsec,label,analysisDir,label,isMc,monitorOption),options.dryRun)
