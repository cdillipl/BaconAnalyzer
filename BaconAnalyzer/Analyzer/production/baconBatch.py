...
def write_job(exec_line, out, analyzer, i, n, eosout=''):
    ...
    print str(n) + "THIS IS THE NUMBER FILES"
...
for job_i in range(njobs):
  ...
  #          if njobs_per_file > 1:                    
 #               job_exec += exec_line_i + '; %s cp %s %s/%s_job%d_file%d_subjob%d.root; ' % (EOS, outfile, options.eosoutdir, outfile, job_i, fil_i, subjob_i)
           # else:
 
        #else:
            #if njobs_per_file > 1:
 #               job_exec += exec_line_i + '; mv %s %s/%s_job%d_file%d_subjob%d.root; ' % (outfile, options.outdir, outfile, job_i, fil_i, subjob_i)
        else:
